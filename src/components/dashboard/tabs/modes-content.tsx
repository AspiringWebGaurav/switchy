...
import { Check, Sparkles, Zap, Clock, AlertTriangle, Building2, Wifi } from "lucide-react";
...
interface ProjectModesContentProps {
  onRefresh: () => void;
}
...
export function ProjectModesContent({ onRefresh }: ProjectModesContentProps) {
  const { project, mode, setMode, saving, saveStatus, connectionState, loading } = useProject();
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-zinc-500">Loading mode settings...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      {/* SSE Connection Indicator (Debug Mode) */}
      {typeof window !== "undefined" && (window as unknown as { __SWITCHY_DASHBOARD_DEBUG__?: boolean }).__SWITCHY_DASHBOARD_DEBUG__ && (
        <div className="fixed bottom-4 right-4 z-50 bg-zinc-900 text-white text-xs font-mono p-3 rounded-lg shadow-lg">
          <div className="flex items-center gap-2 mb-1">
            <Wifi size={12} className={connectionState === "connected" ? "text-emerald-400" : "text-amber-400"} />
            <span>SSE: {connectionState}</span>
          </div>
          <div>Mode: {mode}</div>
      ...
      >
        ...
        <div>
          <p className="text-xs font-medium text-stone-700">Real-time sync enabled</p>
