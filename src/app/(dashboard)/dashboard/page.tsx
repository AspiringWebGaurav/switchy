...
const VALID_TABS: TabId[] = ["overview", "keys", "modes", "templates", "events", "settings"];
...
export default function DashboardPage() {
  ...
  const activeTab = useMemo(() => {
    return urlTab && VALID_TABS.includes(urlTab) ? urlTab : "overview";
  }, [urlTab]);
  ...
  useEffect(() => {
    if (selectedProjectId && !loading) {
      ...
      const effectiveTab = currentUrlTab && VALID_TABS.includes(currentUrlTab) ? currentUrlTab : "overview";
  ...
  useEffect(() => {
    ...
    if (isOnboarding || isDetected) {
      ...
      // Auto-stop after 2 minutes to protect serverless function quota if tab is left idle
      const timeout = setTimeout(() => {
